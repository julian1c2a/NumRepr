...
digit_variant::UINT_T digit_variant::get() const noexcept
{
    const auto fn = []<std::uint64_t k>(const dig_t<k> &arg) -> digit_variant::UINT_T {
        return arg();
    };
    return std::visit(fn, *this);
}

digit_variant::UINT_T digit_variant::operator()() const noexcept
{
    const auto fn = []<std::uint64_t k>(dig_t<k> arg) -> digit_variant::UINT_T { return (arg()); };
    return std::visit(fn, *this);
}

bool digit_variant::is_unit() const noexcept
{
    return std::visit([]<std::uint64_t k>(const dig_t<k> &arg) -> bool { return (arg.is_unit()); },
                      *this);
}

bool digit_variant::is_0_divisor() const noexcept
{
    return std::visit([]<std::uint64_t k>(
                          const dig_t<k> &arg) -> bool { return (arg.is_0_divisor()); },
                      *this);
}
...
{
    return std::visit([]<std::uint64_t k>(
                          const dig_t<k> &arg) -> digit_variant { return (arg.mult_inv()); },
                      *this);
}
