...
constexpr CharT nullchar{CharT('\0')};
...
constexpr char *clear_ccad(char *cad_c, size_t long_de_cad_c) noexcept
{
...
constexpr ullint_t atoull(char *text) noexcept
{
  int i = 0;
  while (*text) {
    i = (i << 3) + (i << 1) + (*text - '0');
...
constexpr bool is_integral_type_v = std::is_integral_v<INT_TYPE>;
/// CONCEPT DEFINCITION
